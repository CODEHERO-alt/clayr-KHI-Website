...
index 4b22fe73663a920e1845b018693fdecdf10fb73e..224095dfade5a873e6241c39b62bbc0287ddb44b 100644
--- a/src/App.jsx
+++ b/src/App.jsx
@@ -1,60 +1,76 @@
...
@@ -156,495 +172,540 @@ export default function ClayrMonolith() {
   // ------------------------
   const scrollTo = (id) => {
   ...
   // Render monolith
   // ------------------------
+  const navTargets = ["home", "studio", "journey", "workshops", "community", "connect"];
+
   return (
...
+                  <img src={logo} alt="Clayr logo" className="w-full h-full object-cover" />
                 </div>
...
+        <motion.section id="home" ref={heroRef} className="relative min-h-screen flex items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, ease: "easeOut" }}>
...
+              <motion.div className="space-y-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }} viewport={{ once: true }}>
...
+                  <motion.div className="rounded-xl overflow-hidden shadow-2xl" whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 150, damping: 20 }}>
...
+                <motion.div className="absolute -bottom-8 right-2 hidden lg:block" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} viewport={{ once: true }}>
...
+        <motion.section id="studio" className="px-6 md:px-24 py-24 bg-transparent" {...fadeIn}>
           <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
-            <div className="space-y-6">
+            <motion.div className="space-y-6" {...fadeIn} transition={{ ...fadeIn.transition, delay: 0.05 }}>
...
+                    whileHover={{ translateY: -4, boxShadow: "0 12px 30px rgba(0,0,0,0.08)" }}
...
+              <motion.div className="rounded-xl overflow-hidden bg-white border-2 border-[#1E1C1A] shadow-lg" {...floatIn}>
...
+              <motion.div className="rounded-xl overflow-hidden bg-[#E7D6B7] p-6 border border-[#4B3324]" {...floatIn} transition={{ ...floatIn.transition, delay: 0.05 }}>
...
+        <motion.section id="journey" className="py-20 bg-[#E7D6B7]" {...fadeIn}>
...
+        <motion.section id="workshops" className="py-20 px-6" {...fadeIn}>
...
+              {workshops.map((w, idx) => (
+                <motion.div key={w.id} className="p-6 rounded-xl border border-[#E7D6B7] shadow-md bg-white" {...floatIn} transition={{ ...floatIn.transition, delay: idx * 0.05 }} whileHover={{ translateY: -4 }}>
...
+              <motion.div className="p-6 rounded-xl border border-[#E7D6B7] bg-[#F9F5EE]" {...floatIn} transition={{ ...floatIn.transition, delay: 0.1 }}>
...
+        <motion.section id="work" className="py-24 px-6 bg-transparent" {...fadeIn}>
           <div className="max-w-6xl mx-auto">
...
+              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="px-6 py-3 bg-[#C7885B] text-white rounded-full" onClick={() => alert('Open catalog (demo)')}>
+                View Catalog
+              </motion.button>
             </div>
           </div>
...
+
+          {/* ===== Community / Events / Testimonials ===== */}
+          <motion.section id="community" className="py-24 px-6 bg-[#E7D6B7]" {...fadeIn}>
+            <div className="max-w-6xl mx-auto">
+              <h2 className="text-4xl font-extrabold text-center text-[#4B3324] mb-10">{t.community}</h2>
+
+              <div className="grid md:grid-cols-3 gap-8 mb-12">
+                {["Community Market", "Open Critique", "Apprenticeship"].map((title, idx) => (
+                  <motion.div key={title} className="p-6 bg-white border rounded-lg shadow" {...floatIn} transition={{ ...floatIn.transition, delay: idx * 0.05 }} whileHover={{ translateY: -4 }}>
+                    <h3 className="font-bold">{title}</h3>
...
+                    <motion.blockquote key={tt.id} className="bg-white p-4 border rounded" {...floatIn} transition={{ ...floatIn.transition, delay: idx * 0.05 }}>
...
+                  {team.map((m, idx) => (
+                    <motion.div key={m.id} className="flex items-center gap-4 bg-white p-4 border rounded" {...floatIn} transition={{ ...floatIn.transition, delay: idx * 0.05 }}>
...
+                {["Local Cafe", "Design Collective", "Gallery", "Material Supplier"].map((partner, idx) => (
+                  <motion.div key={partner} className="p-4 bg-white border rounded" {...floatIn} transition={{ ...floatIn.transition, delay: idx * 0.05 }}>
+                    {partner}
+                  </motion.div>
                 ))}
               </div>
...
+                <p className="text-sm mt-2 text-[#4B3324]">Reach out for custom timelines, private sessions or wholesale partnerships. We respond within one business day.</p>
               </div>
             </div>
-          </div>
-        </section>
...
+          {/* ===== Newsletter / CTA ===== */}
+          <motion.section className="py-16 bg-[#4B3324] text-[#F9F5EE] px-6" {...fadeIn}>
...
+            <motion.form onSubmit={handleNewsletter} className="flex flex-col sm:flex-row gap-3 justify-center" initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} viewport={{ once: true }}>
...
-            </form>
+              <motion.button type="submit" className="px-6 py-3 bg-[#DCB34A] font-bold rounded" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
+                {t.send}
+              </motion.button>
+            </motion.form>
           </div>
...
+        <motion.section id="connect" className="py-20 px-6" {...fadeIn}>
...
+                <motion.div className="bg-white p-4 border rounded" whileHover={{ translateY: -3 }} transition={{ type: "spring", stiffness: 160, damping: 18 }}>
...
+                    <motion.button className="px-4 py-2 bg-[#4B3324] text-white rounded" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>Send</motion.button>
+                    <motion.a className="px-4 py-2 border border-[#4B3324] rounded" href="https://wa.me/" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>{t.whatsapp}</motion.a>
                   </div>
                 </form>
               </div>
-            </div>
...
+                  <img src={logo} alt="Clayr icon" className="w-full h-full object-cover" />
                 </div>
...
+              >
+                <button onClick={() => setSelectedImage(null)} className="absolute -top-10 right-0 text-white p-2 rounded">Close</button>
+                <img src={selectedImage.img} alt={selectedImage.title} className="w-full h-auto max-h-[80vh] object-contain rounded" />
+                <div className="mt-4 bg-white p-4 rounded">
+                  <div className="font-bold">{selectedImage.title}</div>
+                  <div className="text-sm text-[#4B3324]">Item details and story. Replace with real copy for production.</div>
